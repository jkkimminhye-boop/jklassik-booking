cat > "app/api/bookings/create/route.ts" << 'EOF'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('bookings').insert([{ booking_no: bookingNo, student_name, phone_number, email, education_level, major, consultation_content, consultation_date, consultation_time, status: 'pending', token, token_expires_at: tokenExpires.toISOString() }]);
